...
class Client extends _Modules_1.AbstractService {
  constructor(apiKey, useSandboxEnv) {
    ...
    this.config = Object.assign(
      Object.assign({}, this.getDefaultConfig(useSandboxEnv)),
      {
        apiKey: apiKey || null,
        enviroment: useSandboxEnv
          ? types_2.EnviromentEnum.SANDBOX
          : types_2.EnviromentEnum.LIVE
      }
    );
    this.validateConfig();
    const { apiBase } = this.config;
    this.prepareModules(apiBase);
    this.clients = [this.public, this.order, this.refunds];
  ...
  prepareModules(apiBase) {
    ...
    this.order = new _Modules_1.orderClient(apiBase);
  }
  ...
  validateConfig(config) {
    ...
    if (typeof apiBase !== 'string') {
      throw new _Exception_1.InvalidArgumentException(
        'apiBase must be a string'
      );
    }
    if (
      ![types_2.EnviromentEnum.LIVE, types_2.EnviromentEnum.SANDBOX].includes(
        enviroment
      )
    ) {
      throw new _Exception_1.InvalidArgumentException(
        `Environment does not exist. Available environments: ${Object.values(
          types_2.EnviromentEnum
        ).join(', ')}`
      );
    }
  }
